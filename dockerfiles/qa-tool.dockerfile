...
RUN PIPENV_VENV_IN_PROJECT=1 pipenv --python 3.12 sync --categories qa-tool
...
# Copy source
COPY --chown=app:app qa_tool_wsgi.py .
COPY --chown=app:app source/common source/common
COPY --chown=app:app source/qa_tool source/qa_tool

ENV PYTHONUNBUFFERED=TRUE
ENV FLASK_APP=qa_tool_wsgi

EXPOSE 8000

# Run
CMD ["gunicorn", "qa_tool_wsgi:app", "--bind=0.0.0.0:8000"]
