...
RUN PIPENV_VENV_IN_PROJECT=1 pipenv --python 3.12 sync --categories internal-api
...
# Copy source
COPY --chown=app:app internal_api_wsgi.py .
COPY --chown=app:app source/common source/common
COPY --chown=app:app source/internal_api source/internal_api

ENV PYTHONUNBUFFERED=TRUE
ENV FLASK_APP=internal_api_wsgi

EXPOSE 8000

# Run
CMD ["gunicorn", "internal_api_wsgi:app", "--bind=0.0.0.0:8000"]
