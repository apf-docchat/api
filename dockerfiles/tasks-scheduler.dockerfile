...
RUN PIPENV_VENV_IN_PROJECT=1 pipenv --python 3.12 sync --categories tasks-scheduler
...
# Copy source
COPY --chown=app:app tasks_scheduler.py .

# Run
CMD ["celery", "--app=tasks_scheduler", "beat", "-S", "redbeat.RedBeatScheduler", "--loglevel=INFO"]
