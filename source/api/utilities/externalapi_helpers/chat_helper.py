...
def chat_query(query = None, collection_id = None, thread_id = None, selected_file_ids = None, current_role=None):
    try:
...
class ChatManager:
        ...
        try:
            ...
            for i in range(len(self.control_sequence['steps'])):
                ...
                if file_category == ChatFileCategory.TEXT_FILES:
                    def text_generator():
                        #categorise query into search type of all docs
                        if (len(self.text_file_ids) > 1):
                            """ response = json.loads(self.openai_helper.callai_json(
                                        system_content = f"
                                        Categorise the User query into one of the following categories:
                                        1. Search type query: The User is looking for specific information in the files.
                                        2. All docs type query: The User is looking for something which requires looking into all the files.
                                        Provide your response in the JSON format with a field called "category" with the value as "search" or "all_docs".
                                        ",
                                        user_content=self.control_sequence['steps'][i]['user_query'],
                                        user_id=self.user_id
                                    ))
                            query_type = response.get('category', 'search') """
                            if 'metadata' in self.control_sequence['steps'][i]['user_query'].lower():
                                query_type = "all_docs"
                            else:
                                query_type = "search"
                        ...
                        if query_type == 'all_docs':
                            #handle all docs type query. for this build a csv row by row. first just send the query to callai_json like above to get header row. then open the files one by one and send full file content with user query and get one row per file. keep appending to the csv till the last file. this way the full csv is obtained. return the csv as the response
                            return self.build_csv_from_files()
                        else:
                            #handle search type query
                            self.prepare_context()
                            self.fetch_previous_chat_history()
                            return self.generate_response()
                    generators.append(text_generator)
                elif file_category == ChatFileCategory.SPREADSHEET_FILES:
                    def spreadsheet_generator():
                        doc_analysis = DocAnalysis(self.organization_id, self.orgname, self.user_id)
                        """ if self.current_step_query == None:
                            self.current_step_query = self.control_sequence['steps'][i]['user_query'] """
                        self.current_step_query = f"""
                            Output of Previous Step:
                            {self.previous_step_output}
                            User query:
    ...
    def build_csv_from_files(self):
        ...
        try:
            ...
            header_response = json.loads(self.openai_helper.callai_json(
                system_content=f"""Look at the User query and from it elicit the key information sought from the files. Return it as a list of field names sought. Ensure that identifying information like Name and email id are included in the field list.
    ...
    def decide_control_flow(self):
        try:
            if ((len(self.spreadsheet_file_ids) > 0 or len(self.tables) > 0 or len(self.gsheet_ids_and_worksheets) > 0) and len(self.text_file_ids) > 0):
                no_context_answer = self.no_context_answer()
                if (no_context_answer):
                    self.answer = no_context_answer
                    self.control_sequence = {"steps": []}
                else:
                    system_content = f"""
                        Your Task:
                        Break down the User query into smaller Steps to execute in sequence.
                        For each step, identify the Category of Files to be used and the modified smaller User query which is being answered in this Step.
            ...
            elif ((len(self.spreadsheet_file_ids) > 0 or len(self.tables) > 0 or len(self.gsheet_ids_and_worksheets) > 0) and (len(self.text_file_ids)== 0)):
                no_context_answer = self.no_context_answer()
                if (no_context_answer):
                    self.answer = no_context_answer
                    self.control_sequence = {"steps": []}
                    ...
                    query = f"""
                        ...
                        {self.query}
                    """
                    self.control_sequence = {"steps": [{"user_query": query, "file_category": f"{ChatFileCategory.SPREADSHEET_FILES}"}]}
                    logger.info(f"Control sequence loaded: {self.control_sequence}")
            elif (len(self.text_file_ids) > 0):
                """ system_content = f"
                    Your Task:
                    Break down the User query into smaller Steps to execute in sequence.
                    For each step, identify the Category of Files to be used and the modified smaller User query which is being answered in this Step.
                    ...
                    3. Query: look at interview scores and compare one person to another person for developer role?
                    Output: {{"steps": [{{"user_query": "give each person's interview score as a table", "file_category": "{ChatFileCategory.TEXT_FILES}"}}, {{"user_query": "compare one person to another person for developer role?", "file_category": "{ChatFileCategory.TEXT_FILES}"}}]}}
                    
                    4. Query: Share the score of the candiate who studied in xyz college?
                    Output: {{"steps": [{{"user_query": "List the candidates who studied in xyz college", "file_category": "{ChatFileCategory.TEXT_FILES}"}}, {{"user_query": "Share the score of the candidate who studied in xyz college?", "file_category": "{ChatFileCategory.TEXT_FILES}"}}]}}

                    5. Query: translate that to hindi
                    Output: {{"steps": [{{"user_query": "translate that to hindi", "file_category": "{ChatFileCategory.PREVIOUS_OUTPUT}"}}]}}

                    6. Query: What is the Product description for Product ?
                    Input files: ["Product Description.pdf", "Sales.pdf"]
                    Output: {{"steps": [{{"user_query": "What is the Product description for Product ?", "file_category": "{ChatFileCategory.TEXT_FILES}"}}]}}

                    7. Query: List the file names.
                    Input files: ["Product Description.pdf", "Sales.pdf"]
                    Output: {{"steps": [{{"user_query": "List the file names.", "file_category": "{ChatFileCategory.PREVIOUS_OUTPUT}"}}]}}
                "
                ...
                self.control_sequence = {"steps": [{"user_query": self.query, "file_category": f"{ChatFileCategory.PREVIOUS_OUTPUT}"}]}
        except Exception as e:
            logger.error(f"Control sequence set to default due to error: {e}")
            self.control_sequence = {"steps": [{"user_query": self.query, "file_category": f"{ChatFileCategory.TEXT_FILES}"}]}

    def no_context_answer(self):
        system_content = f"""
                Your Task:
                Fill a JSON format with a field called "answer". If the User Query is completely and comprehensively answerable using the Context below, go ahead and answer it. If the answer is better given based on the contents of the file, give the "answer" field as "". 
                Use appropriate HTML tags to beautify the answer.
                ...
                Answer: {{"answer": "The files are: <files listed using HTML>"}}

                Query: summarise the files
                Answer: {{"answer": ""}}

                Query: share details of the files
                Answer: {{"answer": ""}}
                ...
                Query: list the columns of the table
                Answer: {{"answer": ""}}
            """
        ...
        logger.debug(f"################Response from pre-processing: {response}")
